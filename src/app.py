...
@st.cache_resource
def load_data():
    data = pd.read_parquet('Data/churn_data_regulated.parquet')
    return data
...
def plot_shap_values(model, explainer, shap_values_cat_train, shap_values_cat_test, customer_id, X_test, X_train):
    ...
    customer_index = X_test.reset_index(drop=True).index[X_test['customerID'] == customer_id].tolist()[0]
...
with st.sidebar:
    selected = option_menu("Navigation", ['User-based SHAP', 'Feature Importance','Calculate the probability of CHURN'], menu_icon="cast", default_index=1)
model = load_model()
data = load_data() 
if selected == "User-based SHAP":
    available_customer_ids = X_test['customerID'].tolist()
    customer_id = st.selectbox("Choose the Customer", available_customer_ids)
    y_pred = model.predict(X_test)
    print(y_pred)
    df_info = pd.DataFrame([list(X_test['customerID']),y_test,y_pred]).T
    df_info.columns = ['customerID','y_test','y_pred']
    customer_index = df_info[df_info['customerID'] == customer_id]
    y_test_value = customer_index['y_test'].values
    y_pred_value = customer_index['y_pred'].values

    if customer_id:
        col1,col2 = st.columns(2)
        with col1:
            st.write(f'Customer {customer_id}: Actual value for the Customer Churn : {y_test_value}')
        with col2:
            st.write(f"Customer {customer_id}: CatBoost Model's prediction for the Customer Churn : {y_pred_value}")
...
if selected=='Calculate the probability of CHURN':
